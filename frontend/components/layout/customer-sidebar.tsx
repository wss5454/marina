...
const navItems = [
  ...
  { href: "/requests/new?form_type=GENERAL", label: "New request", icon: PlusCircle },
  { href: "/reservations/new", label: "Reservation", icon: CalendarDays },
  { href: "/availability", label: "Availability", icon: Anchor },
...
function NavLink({
  ...
  const active = exact ? pathname === pathOnly : pathname?.startsWith(pathOnly);
    ...
    <div className="flex h-full flex-col">
      ...
      >
        {!collapsed && (
          <div className="min-w-0">
            ...
            <p className="truncate text-xs text-primary-foreground/60">Rhode River Marina</p>
          </div>
        )}
        <Button
          variant="ghost"
      ...
      <div className="mt-auto border-t border-primary-foreground/10 p-3">
        <Link
          href="/"
          className={cn(
            "block rounded-md bg-primary-foreground/10 px-3 py-2 text-xs text-primary-foreground/80 transition-colors hover:bg-primary-foreground/15 hover:text-primary-foreground",
        ...
        <SheetContent side="left" className="w-72 border-r-0 p-0 marina-gradient text-primary-foreground">
          <div className="flex h-full flex-col pt-2">
            <div className="px-4 pb-2">
              ...
              <p className="text-xs text-primary-foreground/60">Rhode River Marina</p>
            </div>
